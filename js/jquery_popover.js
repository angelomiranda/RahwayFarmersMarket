	...
	var methods = {
		...
		content: function(html) {
			return this.each(function() {
				...
				if(data) {
					var content = data.popover.find('.pop_content');
					var wrap = data.popover.find('.wrap');
					if(content.length === 0)
						content = $('<div class="pop_content" />').appendTo(wrap);
					content.html(html);
				}
			});
		},
	...
	$(function(){
		$('[rel="popover"]').on('click', function(e){
			...
			if (($this).hasClass('fb-button')){
				$('.popover').hide();
				var href = encodeURIComponent($(this).attr('data-href')); 				// link of facebook
				var layout = $(this).attr('data-layout');								// layout of facebook - standard, button_count, box-count
				var sendButton = $(this).attr('data-send');								// boolean - true or false
				var fbWidth = 'width:' + $(this).attr('data-width') +'px;';				// weight that will be appended in cssStyle below
				var fbHeight = 'height:' + $(this).attr('data-height')+'px;';			// height that will be appended in cssStyle below
				var showFaces = $(this).attr('data-show-faces'); 						// boolean - true or false
				var font = $(this).attr('data-font');									// font - arial, lucida grande, segue ui, tahoma, trebuchet ms, verdana
				var colorscheme = $(this).attr('data-colorscheme');						// light or dark
				var direction = $(this).attr('data-direction');							// direction of pop box
				var cssStyle = 'style="border:none; width:80px; height:21px; padding: 0; margin: 0;"'			// css styling for facebook like button
				var $this = $(this);
				...
				$(this).popover({
					...
					autoReposition: true,
					anchor: false,
					content: function() { return '<iframe src="http://www.facebook.com/plugins/like.php?href='+href+'&amp;layout='+layout+'&amp;data-send='+sendButton+'&amp;width=450&amp;show_faces='+showFaces+'&amp;action=like&amp;colorscheme='+colorscheme+'&amp;font='+font+'&amp;height=26" scrolling="no" frameborder="0" '+ cssStyle +' allowTransparency="true"></iframe>' }
				});
				$(this).popover('show');
			} else {
				$('.popover').hide();
				var twt_count = encodeURIComponent($(this).attr('data-twt_count')); 	// displays twitter count next to twitter button
				var twt_text = encodeURIComponent($(this).attr('data-twt_text')); 		// appends this text when twitter is button clicked
				var twt_url = encodeURIComponent($(this).attr('data-twt_url')); 		// appends this url when twitter is button clicked
				var direction = $(this).attr('data-direction');							// direction of pop box
				var cssStyle = 'style="border:none; width: 57px; height:21px; padding: 2px;"'	// css styling for twitter like button
				$(this).popover({
					position: direction,
					verticalOffset: -10,
					horizontalOffset: -10,
					...
					animateChange: true,
					autoReposition: true,
					content: '<iframe src="http://platform.twitter.com/widgets/tweet_button.html?counturl='+twt_count+'&amp;count=horizontal&amp;text='+twt_text+'&amp;url='+twt_url+'&amp;via=UJAFedNY"'+cssStyle+'" allowtransparency="true" frameborder="0" scrolling="no" ></iframe>'
				});
				$(this).popover('show');
			}
		e.preventDefault();
